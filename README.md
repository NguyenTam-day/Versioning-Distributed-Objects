...
## ⚔️ Test Xung đột (Conflict) & Giải quyết Xung đột

Hệ thống cung cấp một script test tự động kịch bản xung đột phiên bản (split-brain) và tự động giải quyết (conflict resolution):

```bash
# Chạy kịch bản test xung đột giữa Node A và Node B
node test_conflict.js
```

### Kịch bản giả lập chi tiết:
1. **Khởi tạo**: Bật tính năng đồng bộ (`syncEnabled = true`) trên cả 2 node.
2. **Base Version**: Upload phiên bản gốc `v1_A` lên Node A và đợi nó tự động đồng bộ sang Node B.
3. **Mất kết nối (Offline)**: Tắt tính năng đồng bộ (`syncEnabled = false`) trên cả 2 node để mô phỏng phân tách mạng.
4. **Sửa đổi song song**:
   - Client A sửa hình học và upload `v2` lên Node A (được lưu cục bộ dưới tên `v2_A`, parent: `v1_A`, trạng thái: `PENDING_SYNC`).
   - Client B đồng thời sửa hình học và upload `v2` lên Node B (được lưu cục bộ dưới tên `v2_B`, parent: `v1_A`, trạng thái: `PENDING_SYNC`).
5. **Tái kết nối mạng**: Bật lại tính năng đồng bộ trên cả 2 node.
6. **Đồng bộ & Giải quyết**: Kích hoạt lệnh đồng bộ thủ công để đẩy phiên bản từ Node A sang Node B. Hệ thống sẽ phát hiện ra `v2_A` và `v2_B` cùng chung parent `v1_A` và tiến hành giải quyết xung đột dựa trên cơ chế **First-Writer-Wins (FWW)**:
   - Phiên bản có timestamp nhỏ hơn (được tạo trước) sẽ chiến thắng (**Winner**) → Giữ nguyên ở nhánh `main` và đánh dấu `SYNCED`.
   - Phiên bản có timestamp lớn hơn (được tạo sau) sẽ thua (**Loser**) → Được tự động chuyển sang nhánh conflict riêng biệt (`conflict/2_B` hoặc `conflict/2_A`) và đánh dấu `CONFLICT`.
7. **Kiểm tra**: Verify dữ liệu lịch sử trên cả 2 node hoàn toàn trùng khớp và thống nhất.
