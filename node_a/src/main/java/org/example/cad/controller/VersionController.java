...
public class VersionController {
    ...
    /**
     * GET /api/version/{modelId}/chain
     * Returns the ordered version chain showing Snapshot vs Delta for each entry.
     */
    ...
    /**
     * GET /api/version/{modelId}/restore/{versionNumber}
     * Walks the delta chain back to the nearest snapshot and applies deltas.
     * Returns the restore steps log and the reconstructed geometry.
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> restoreVersion(
        ...
        // Walk chain: collect deltas from target back to nearest snapshot
        ...
        // Reconstruct geometry and measure time
        ...
        // Snapshot-only restore time (just parse the snapshot)
        long snapshotMs = 0;
        if (baseSnapshot != null) {
    ...
    /**
     * GET /api/version/{modelId}/benchmark/{versionNumber}
     * Compares storage size (snapshot vs delta chain) and restore time.
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> benchmark(
        ...
        // Restore time via delta chain
        long t0 = System.currentTimeMillis();
        org.example.dv.Geometry3D reconstructed = versionService.reconstructGeometry(target);
        long deltaRestoreMs = System.currentTimeMillis() - t0;

        // Restore time snapshot-only (parse snapshot geometry directly)
    ...
    /**
     * GET /api/version/{modelId}/delta/{versionNumber}
     * Returns the raw diff changes stored in a delta version (vertex/face modifications).
     */
